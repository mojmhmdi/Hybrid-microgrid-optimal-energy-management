function [cost_second,Plosss,Volt_dev,Ploss,Voltage,Angle_Voltage,Psub,qsub,ploss,psub1,hourly_cost]=Cost_eval(ipop,DG_Min_Max_Bid,Bid_Market,Intervals,Wind_pwr_Forecast,PV_pwr_Forecast,Branch_data,Bus_data,WT1,WT2,WT3,PV2,PV,Ld_Fact,phev_demand)
...
cost_PV2=(ipop_PV2.*DG_Min_Max_Bid(1,4));
cost_WT3=(ipop_WT3.*DG_Min_Max_Bid(2,4));
cost_PV=(ipop_PV.*DG_Min_Max_Bid(1,4));
cost_WT=(ipop_WT.*DG_Min_Max_Bid(2,4));
cost_FC=(ipop_FC.*DG_Min_Max_Bid(3,4));
cost_MT=(ipop_MT.*DG_Min_Max_Bid(4,4));
cost_Bat=(ipop_Bat.*DG_Min_Max_Bid(5,4));
cost_MT2=(ipop_MT2.*DG_Min_Max_Bid(7,4));
cost_MT3=(ipop_MT3.*DG_Min_Max_Bid(8,4));
cost_WT2=(ipop_WT2.*DG_Min_Max_Bid(2,4));
...
for i=1:Intervals
    ...
    [Ploss(i),Voltage(i,:),Angle_Voltage(i,:),Psub(i),Qsub(i)]=loadflowDG(Branch_data_new,Bus_data_new);
    psub(i)=Psub(i)*1000000;
    qsub(i)=Qsub(i)*1000000;
...
hourly_cost=cost_PV+cost_WT+cost_FC+cost_MT+cost_Bat+cost_Market+cost_MT2+cost_MT3+cost_WT2+cost_WT3+cost_PV2;
Plosss=sum(Ploss);
Volt_dev;
Psub;
psub1=psub;
